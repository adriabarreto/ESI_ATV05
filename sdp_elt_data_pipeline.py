...
def tansform_raw_dataset(dataset_file_path: str=None) -> None:
    ...
    dataset_raw = pd.read_csv(dataset_file_path, sep=';')

    # Removendo arquivos com example ou exercise com a função contains
    filter_mask = dataset_raw[dataset_raw['FILE'].str.contains('example')].index
    dataset_filter = dataset_raw.drop(filter_mask, axis=0)
    filter_mask = dataset_filter[dataset_filter['FILE'].str.contains('exercise')].index
    dataset_filter = dataset_filter.drop(filter_mask, axis=0)

    # Removendo arquivos que a maioria dos atributos estejam zerados (sem contar com a predição)
    colunas = list(dataset_filter.columns)
    colunas.pop(0)
    colunas.pop()
    zeros_por_linha = (dataset_filter[colunas] == 0).sum(axis=1)
    dataset_filter_zeros = dataset_filter[zeros_por_linha < len(colunas) // 2]

    # Removendo outliers
    # Criando função para remover outliers usando o z-score
    def remove_outliers_zscore(df, columns, threshold=3):
        """
        Remove outliers usando Z-Score.
        threshold: número de desvios padrão (3 é o padrão)
        """
        df_clean = df.copy()
        for col in columns:
    ...
    dataset_filtered_final = remove_outliers_zscore(dataset_filter_zeros, colunas)
    
    dataset_filtered_final.to_csv(transformed_file_path, sep=";")
